...
class DroneVisualizer:
    def __init__(self, simulation, scale=60):
        pygame.init()
        self.sim = simulation
        self.scale = scale
        self.offset_x = 100
    ...
    def draw_map(self):
        ...
        for zone_name, connections in self.sim.graph.adj.items():
            ...
            for conn in connections:
                end_pos = self.to_screen(self.sim.graph.zones[conn.target_zone].coords)
                pygame.draw.line(self.screen, (60, 60, 60), start_pos, end_pos, 2)
        ...
        for zone in self.sim.graph.zones.values():
            ...
            pygame.draw.circle(self.screen, (255, 255, 255), pos, 15, 2) # Borde
            
            # Etiqueta de la zona y capacidad
            label = self.font.render(f"{zone.name} ({zone.max_drones})", True, (200, 200, 200))
            self.screen.blit(label, (pos[0] - 20, pos[1] + 20))

    def draw_drones(self):
        for drone in self.sim.drones:
            if not drone.is_active: continue
            
            # Dibujamos al dron en su zona actual
            pos = self.to_screen(self.sim.graph.zones[drone.current_zone].coords)
            
            # Si está en movimiento restringido, lo pintamos un poco desplazado o diferente
    ...
    def run_step(self):
        ...
        pygame.display.flip()
        self.clock.tick(10) # 10 FPS para que nos dé tiempo a ver los movimientos