...
class Parser():
        ...
        try:
            with open(self.file_name, 'r') as file:
                for line_num, line in enumerate(file, 1):
                    ...
                    metadata = self._extract_metadata(rest)
                    pos_bracket = rest.find("[")
                    payload = rest[:pos_bracket] if pos_bracket != -1 else rest
                    data_list = payload.strip().split()
    ...
    def _handle_nb_drones(self, data: List[str], line_num: int):
        try:
            count = int(data[0])
            if count <= 0:
                raise ValueError
    ...
    def _handle_hub(self, prefix: str, data: List[str],
                    meta: Dict, line_num: int):
        if len(data) < 3:
        ...
        z_type = type_mapping.get(meta.get("zone", "normal"), TypeZone.normal)
    ...
    def _handle_connection(self, data: List[str], meta: Dict, line_num: int):
    ...
    def _error(self, message: str, line_num: int):
